...
public class PaloLibUtil {
    ...
    public static boolean setInput(ICube cube, IElement [] coordinate, Object input, char thousandSeparator, char decimalPoint) {
        if (input == null) {
                return false;
        }
    ...
    public static boolean copyCell(ICube cube, IElement[] src, IElement[] dest) { //TODO implement real copy like
        try {
    ...
    private static IElement[] parseCopyLikeParameter(ICube cube, String params) {
        try {
    ...
    public static synchronized Map<String,ICell> buildCellLookup(ICellsExporter export) {
        Map<String,ICell> result = new HashMap<>();
