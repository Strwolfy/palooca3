        ...
        private int drawSection(Graphics g, int y, int sectionIndex, String label) {
            ...
            if (selectedIndex == sectionIndex && openIndex == sectionIndex && dimension != null) {
                ...
                for (Subset2 subset : subsets) {
                    if ((sectionIndex == 1 && subset.getType() == Subset2.TYPE_LOCAL) ||
                            (sectionIndex == 2 && subset.getType() == Subset2.TYPE_GLOBAL)) {

                        img = new ImageIcon(Objects.requireNonNull(getClass().getResource("/images/subset.png"))).getImage();
                        g.drawImage(img, 3, y + (itemHeight - 16) / 2, this);

                        if (subset == selectedSubset) {
                            Rectangle2D rtext = fm.getStringBounds(subset.getName(), g);
                        ...
                        g.drawString(subset.getName(), 25, y + yTxtOffset);
