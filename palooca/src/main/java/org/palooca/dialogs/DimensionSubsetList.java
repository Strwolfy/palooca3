...
public class DimensionSubsetList extends JPanel
    ...
    private static class FilterElementActionListener implements ActionListener {
        private final ElementSingleSelectDialog browser;
    ...
    private final Vector<DimensionSubsetListItem> dimensions = new Vector<>();
    ...
    public class DrawingPane extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (int i = 0; i < dimensions.size(); i++) {
                ...
                Image img;
                if (dimItem.isOpen()) {
                    img = new ImageIcon(Objects.requireNonNull(getClass().getResource("/images/panelClose.PNG"))).getImage();
                } else {
                    img = new ImageIcon(Objects.requireNonNull(getClass().getResource("/images/panelOpen.PNG"))).getImage();
                }
                ...
                if (selectedIndex == i && dimItem.isOpen()) {
                    ...
                    if (dimItem.getSubset() == null) {
                        img = new ImageIcon(Objects.requireNonNull(getClass().getResource("/images/radioChecked.PNG"))).getImage();
                    } else {
                        img = new ImageIcon(Objects.requireNonNull(getClass().getResource("/images/radioUnchecked.PNG"))).getImage();
                    }
                    g.drawImage(img, 4, y + (itemHeight - 13) / 2, this);
                    g.drawString(ResourceBundle.getBundle("org/palooca/dialogs/PalOOCaDialogs").getString("SelectElements"), 20, y + yTxtOffset);

                    y += itemHeight;

                    for (Subset2 subset : subsets) {
                        if (dimItem.getSubset() == subset) {
                            img = new ImageIcon(Objects.requireNonNull(getClass().getResource("/images/radioChecked.PNG"))).getImage();
                        } else {
                            img = new ImageIcon(Objects.requireNonNull(getClass().getResource("/images/radioUnchecked.PNG"))).getImage();
                        }
                        ...
                        g.drawString(subset.getName(), 20, y + yTxtOffset);
